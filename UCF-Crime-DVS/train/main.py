...
if __name__ == '__main__':
    ...
    # Reduce workers for low-end CPU
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, pin_memory=False,
                              num_workers=0, shuffle=True)
    test_dataset = dataset(args=args, train=False)
    train2test_dataset = dataset_train2test(args=args)
    test_loader = DataLoader(dataset=test_dataset, batch_size=1, pin_memory=False,
                             num_workers=0, shuffle=False)
    train2test_loader = DataLoader(dataset=train2test_dataset, batch_size=1, pin_memory=False,
                                   num_workers=0, shuffle=False)
