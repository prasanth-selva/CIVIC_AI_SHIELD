...
class vit_snn(nn.Module):
    def __init__(self,
                 img_size_h=128, img_size_w=128, patch_size=16, in_channels=2, num_classes=14,
        ...
        self.patch_embed = SpikingTokenizer(img_size_h=img_size_h,
                          img_size_w=img_size_w,
                          patch_size=patch_size,
                          in_channels=in_channels,
                          embed_dims=embed_dims)
        self.num_patches = self.patch_embed.num_patches
        self.block = nn.ModuleList([SpikingTransformer(
        ...
        # no need classification head
        # self.head = nn.Linear(embed_dims, num_classes) if num_classes > 0 else nn.Identity()

        # self.norm = norm_layer(embed_dims)  # Apply normalization to the last dimension
    ...
    def forward_features(self, x):
        ...
        for blk in self.block:
            x = blk(x)
        x = x.flatten(3).mean(3)  # 将特征展平并在空间维度上取平均 (16,16,256,8,8) -> (16,16,256)
        # x = self.norm(x)  # Apply the normalization
        return x
...
def Spikingformer(pretrained=False, **kwargs):
    model = vit_snn(
        ...
        in_channels=2, num_classes=14, qkv_bias=False,
